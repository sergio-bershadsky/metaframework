...
`kit-works-without-the-spec` requirement, and cured only when the skill
stopped describing somebody's test run at all and named `metaframework check`.
A distinct failure mode is the third component test passing — which is
precisely why the reference bundle is a component and the skills are not.
