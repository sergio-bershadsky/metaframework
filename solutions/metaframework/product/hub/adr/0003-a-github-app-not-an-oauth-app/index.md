...
- **The blast radius of this server being compromised is what owners granted
  it, and nothing else.** That is the whole decision. A hub holding user tokens
  would be a credential store worth attacking; a hub holding a private key that
...
- **The first run is worse, and this is the honest cost.** A reader cannot just
  ...
  [repo-sync](srn://metaframework/product/hub/component/repo-sync).
...
- **Tokens must not reach git's config or its logs.** Consequence enforced in
  [repo-sync](srn://metaframework/product/hub/component/repo-sync): credentials
  ...
  [telemetry](srn://metaframework/product/hub/component/telemetry) names them
  for the same reason.
