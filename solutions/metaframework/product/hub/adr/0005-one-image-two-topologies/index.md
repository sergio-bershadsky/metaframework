...
- **`docker/` is a stated constraint, so it is a requirement rather than a
  convention.**
  [deployment-files-live-under-docker](srn://metaframework/product/hub/requirement/deployment-files-live-under-docker)
...
- **A single Dockerfile running everything under a supervisor.** One image, one
  ...
  [worktree-lease](srn://metaframework/product/hub/protocol/worktree-lease)
