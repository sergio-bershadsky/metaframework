...
relations:
  exposes:
    - /product/hub/protocol/worktree-lease
  depends-on:
    - ../github
    - ../telemetry
  implements:
    - /product/hub/requirement/any-git-repository-is-a-catalog-source
    - /product/hub/requirement/git-state-survives-a-restart
  realizes:
...
[0001-a-worktree-per-branch](srn://metaframework/product/hub/adr/0001-a-worktree-per-branch).
...
One protocol,
[worktree-lease](srn://metaframework/product/hub/protocol/worktree-lease): the
[catalog-router](srn://metaframework/product/hub/component/catalog-router) asks
...
through them fills the volume, and a full volume is not a degraded hub — it is a
hub where `git fetch` fails, which is indistinguishable from a hub that serves
the wrong thing.
...
[github](srn://metaframework/product/hub/component/github) for a short-lived
