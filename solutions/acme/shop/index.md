...
- [checkout](srn://acme/shop/checkout) — cart to order, tax quoting, payment
  orchestration. The only component a customer's browser talks to.
- [inventory](srn://acme/shop/inventory) — stock availability and the
  reservation that holds it during a checkout attempt.

Beneath checkout sit two more: [payment](srn://acme/shop/checkout/payment),
which owns the conversation with the card acquirer, and
[tax-engine](srn://acme/shop/checkout/tax-engine), a library that runs inside
checkout's own process. Nesting here is composition, not dependency — payment is
*part of* checkout, whereas checkout's need for
[ledger](srn://acme/billing/ledger) is an edge that crosses the product
boundary.

## Public surface

The product's own public surface is one datamodel,
[order-placed](srn://acme/shop/datamodel/order-placed@1) — the fact other
...
[ledger](srn://acme/billing/ledger) is reuse by reference: the component stays in
